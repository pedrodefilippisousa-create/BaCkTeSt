...
O script baixa os dados do Yahoo Finance, roda o backtest setorial e
**abre o relatório automaticamente no navegador**. Ele também salva o arquivo
`setores_b3_dashboard.html` na mesma pasta.
...
## Para mudar os setores ou as ações

Abra o `Backtest.py` num editor de texto, edite o bloco `SECTORS` no início do
arquivo (setores e as ações de cada um), salve e rode de novo. Lembrando:

- Os tickers são da **B3 sem o sufixo `.SA`** (ex.: `PETR4`, `VALE3`) — o script
  adiciona o `.SA` sozinho.
- Você pode adicionar/remover setores inteiros ou ações dentro de cada setor.
- Ações sem dados no Yahoo são ignoradas com um aviso (não quebra o relatório).
